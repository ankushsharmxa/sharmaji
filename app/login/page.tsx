...
function LoginContent() {
  ...
  // Custom inline zod resolver to prevent external resolver package issues
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      await authService.googleLogin(true); // default remember me to true for Google Login
    } catch (err: any) {
      setErrorMessage(getFriendlyErrorMessage(err));
    } finally {
      ...
      <Card className="w-full max-w-md p-6 sm:p-8 bg-white relative">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2 mt-1">
            <input
              ...
              {...register("rememberMe")}
            />
            <label htmlFor="rememberMe" className="text-xs font-bold text-gray-600 select-none">
        ...
        >
          {isGoogleLoading ? (
            ...
            <>
              <svg className="w-4 h-4 mr-1.5" viewBox="0 0 24 24" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
